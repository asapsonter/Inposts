document.addEventListener("DOMContentLoaded", () => {
  ...
  // --- Catalog selection/delete elements ---
  ...
  async function pickArticle(article) {
    const confirmed = confirm(
      `Generate and publish a LinkedIn post about:\n\n"${article.title}"\n\nThis will publish to LinkedIn immediately.`,
    );
    if (!confirmed) return;
    closePicker();
  ...
  function renderPosts(posts) {
    ...
    lastPosts = posts;
    postsContainer.innerHTML = "";
    catalogToolbar.classList.toggle("hidden", posts.length === 0);
    ...
    posts.forEach((post, index) => {
      ...
      const selectMarkup = selectionMode
        ? `<input type="checkbox" class="post-select-box" data-id="${post.id}" ${selectedIds.has(post.id) ? "checked" : ""} aria-label="Select post ${post.id}">`
        : "";

      const url = `https://www.linkedin.com/feed/update/${post.linkedin_id}`;
      const isLinkedinIdValid =
        post.linkedin_id && post.linkedin_id.includes("urn:");
  ...
  function enterSelectionMode() {
    ...
    updateDeleteButton();
    renderPosts(lastPosts); // redraw with checkboxes
  }
  ...
  function exitSelectionMode() {
    ...
    renderPosts(lastPosts); // redraw without checkboxes
  }
  ...
  async function deleteSelected() {
    ...
    const ok = confirm(
      `Delete ${ids.length} post${ids.length === 1 ? "" : "s"} from your catalog?\n\n` +
        `This removes them from this dashboard only — it does NOT delete them from LinkedIn.`,
    );
    if (!ok) return;
    ...
    try {
      ...
      exitSelectionMode();
      await fetchPosts(); // reload the now-shorter catalog
    } catch (error) {
  ...
  function renderSchedule(s) {
    ...
    if (s.mode === "hourly") {
      ...
      const days =
        s.interval_days === 1 ? "Every day" : `Every ${s.interval_days} days`;
    ...
    if (s.enabled && s.next_run_at) {
      ...
      schedNext.textContent =
        "Auto-posting is paused — turn the switch on to enable.";
    } else {
      schedNext.textContent = "";
    }
  }
